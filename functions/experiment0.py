...
neurons=128, recurrent_dropout=0.08, dropout=0.01, dense=1, loss= "mse", optimiser = "adam", patience=100):
  ...
  monitor = EarlyStopping(monitor='loss', patience=patience, verbose=1, mode='auto', restore_best_weights=True)
  history = model.fit(X_train,y_train,validation_data=(X_test,y_test),verbose=0, callbacks=[monitor,WandbCallback()],epochs=epochs,batch_size=batch_size)#added batchsize
  # callbacks=[]
  e =dt.datetime.now()
  time = e-s
...
def testmodel2(dataset="litecoin",sequence_length=1,
lookback=1,test_size=0.3, epochs=100000, batch_size=128,
neurons=128, recurrent_dropout=0.08, dropout=0.01, dense=1, loss= "mse", optimiser = "adam", patience=100):
  dataset_data  = datasetLoader("dataset/"+dataset+".csv")
  data = dataset_data.values
...
def testmodel3(dataset="litecoin",sequence_length=1,
lookback=1,test_size=0.3, epochs=100000, batch_size=128,
neurons=128, recurrent_dropout=0.08, dropout=0.01, dense=1, loss= "mse", optimiser = "adam", patience=100):
  dataset_data  = datasetLoader("dataset/"+dataset+".csv")
  data = dataset_data.values
